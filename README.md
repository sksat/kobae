...
起動時は飛行ビュー（体と挙動が主役、脳の点群は右パネル）。体が未接続なら静止姿勢のハエがアリーナに立っている。
上部の「脳」に切り替えると神経系そのものを大きく見られる（選択はブラウザに記憶される）。
...
（脳ビュー。糖受容ニューロンを刺激した状態: 白く光る点が発火した細胞、右の「行動の出力」で吻を伸ばす MN9 が立ち上がる）

<img src="docs/viewer-brain-bar.png" width="800">

（目に横切る縦棒を見せた状態。両側の視葉が光る）
...
- ライブ表示は **BC-250 上で体も動かす**（`--body kinematic`、姿勢 = 指令から作る参照軌道、翅は拍動パターン）。実時間の 0.77 倍で動く。MuJoCo 力学（`--body mujoco`）は 1/13 で、録画・検証用
- ブラウザの飛行ビューは three.js: MuJoCo から書き出したリグ（68 ボディ、25 万三角形、`body/kobae_body/export_rig.py` → `rig.json` + `rig.bin`）を 20 Hz の姿勢ストリームで動かす。目の映像は影・反射なし・自分の体なしで 2 ms/組
